...
namespace Gordian.Core.Network.Crypto
{
    ...
    {
        public string SuiteName => "Blowfish-FFXI-ECB";
        public bool IsKeyInitialized { get; private set; }
        ...
        public LegacyBlowfishCryptoSuite()
        {
            ResetToDefaultSubkey();
        }

        private void ResetToDefaultSubkey()
        {
            Buffer.BlockCopy(FfxiSubkey, 0, _p, 0, 72);
            Buffer.BlockCopy(FfxiSubkey, 72, _s, 0, 4096);
        }
        ...
        public bool TryDecryptAndVerify(Span<byte> packetData, int headerSize, out int decryptedPayloadLength)
        {
            decryptedPayloadLength = 0;
            if (packetData.Length < headerSize + 16)
            {
                return false;
            }

            int totalCipherRegion = packetData.Length - headerSize;
            int numWords = totalCipherRegion / 4;
        ...
        {
            // Compute MD5 over unencrypted payload
            ReadOnlySpan<byte> payloadSpan = datagramBuffer.Slice(headerSize, payloadLength);
            Span<byte> hashDestination = datagramBuffer.Slice(headerSize + payloadLength, 16);
            MD5.HashData(payloadSpan, hashDestination);

            int totalCipherLength = payloadLength + 16;
            int wordCount = (totalCipherLength / 4) & ~1;
            int blockCount = wordCount / 2;

            if (IsKeyInitialized && blockCount > 0)
            {
                Span<byte> cipherBytes = datagramBuffer.Slice(headerSize, wordCount * 4);
                EncipherBlocks(cipherBytes, blockCount);
            }

            return headerSize + totalCipherLength;
        }
        ...
        private void DecipherBlocks(Span<byte> data, int blockCount)
        {
            for (int b = 0; b < blockCount; b++)
            {
                int offset = b * 8;

...
*   **🧩 Sandboxed Lua Scripting & Dear ImGui:** Houses a secure, allowlist-sandboxed Lua (NLua) scripting runtime for full backward compatibility with user-built `GearSwap` logic rules — addon scripts execute inside a restricted environment exposing only a deliberately curated API surface, with no path to the .NET CLR, raw filesystem APIs, or OS process control. Addons instead get a scoped, quota-bound storage API confined to their own data directory for config and logs. It includes a native **ImGui.NET** layer to render modern, rounded, translucent HUD overlays with zero Garbage Collection allocation overhead.

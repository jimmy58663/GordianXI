...
- [ ] **Phase 5E: UI Layering, Stock DAT 2D HUD & ImGui In-Game Overlays:**
  - [ ] **3-Tier Rendering Architecture:**
    ...
    - [x] *Phase 5E (Tier 1) Sky, Weather & Celestial Subsystem (5 Modular Chunks):*
      ...
      - [ ] **Step: Weather Emitters, Point Lights & Effect Audio:** Weather-directory emitters (rain, snow, splashes) need batched sub-particles and camera-space placement; zone point lights (~15.6k generators, e.g. torch light) need dynamic lights in the terrain/entity shaders; effect audio (~5k) needs the sound system. Child generators do not inherit the parent's orientation, and camera-oriented spawn scatter and Movement/Camera billboards are not implemented. Looping ambient routines at their authored total length matched retail side by side on Bibiki Bay (2026-09-24).

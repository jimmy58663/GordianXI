...
- [ ] **Phase 5E: UI Layering, Stock DAT 2D HUD & ImGui In-Game Overlays:**
  - [ ] **3-Tier Rendering Architecture:**
    ...
    - [x] *Phase 5E (Tier 1) Sky, Weather & Celestial Subsystem (5 Modular Chunks):*
      ...
      - [ ] **Step: Weather Emitters, Point Lights & Effect Audio:** Looping ambient routines at their authored total length matched retail side by side on Bibiki Bay (2026-09-24).
        ...
        - [ ] Verify against retail: billboard-None rain cards (e.g. `f_ju/weat/rain/rain`) are drawn flat in world space as the reference does, so they vanish edge-on when looking along X; rain density/alpha (`0x26` colour alpha) not yet compared side by side.
        - [ ] Not yet covered: routine- or client-triggered weather generators (lightning-strike routines `thdr/kmi*`, actor-attached `tobi` effects); weather emitters inside caves should stop (the client checks the viewer's sub-environment); child generators still do not inherit the parent's orientation (`0x47`/`0x79`).
        - [ ] *Chunk 2: Point lights (~15.6k generators).* Binding is documented (xi-tools `docs/zone/format.md`): the ZoneDef light table at header `+0x18` (256 x `0x4C`, generator FourCC at `+0`) allocates a slot per listed id, a `0x47` generator finds its slot by its own FourCC, and each placement's four 1-based light references at `+0x54` are the only lights that shine on it. Params `0x58` (range, theta/"light power", range and theta multipliers `2^x`), clock/progress theta `0x49`/`0x5B`. The attenuation formula is not documented in any available reference.
        - [ ] *Chunk 3: Effect audio (~5.9k generators, `0x3D` link, `0x4C` near/far range, `0x43` clock volume).* Needs the Phase 5H sound system.

...
- [ ] **Phase 5E: UI Layering, Stock DAT 2D HUD & ImGui In-Game Overlays:**
  - [ ] **3-Tier Rendering Architecture:**
    ...
    - [ ] *Tier 2 (Stock FFXI 2D UI):* Authentic DAT-driven menu boxes (blue marble), finger cursor hand, targeting brackets, vitals gauges, status icons, and dialog text.
      ...
      - [ ] **Chunk 2: Tier 2 sprite renderer & layout model.** Veldrid 2D batch in `RenderTier2_StockUi`: layout-to-viewport scaling with corner anchoring, wrap sampling, half-scale colour/alpha modulation, text from `fontshp`. Every persistent window's screen rect comes from one resolver (authored position + anchor corner + optional per-window override: offset from its own anchor corner, scale, hidden), persisted per profile (`ui_layout.json`), clamped on screen, resettable per window or all. No overrides by default, so the authored retail layout is the baseline.
      - [ ] **Chunk 3: Live HUD.** Party/vitals window (HP/MP/TP gauges), target window and targeting brackets, status icons (ROM/280/15), `StockUiVisibilityState`.

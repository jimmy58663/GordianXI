  ...
  - [ ] **3-Tier Rendering Architecture:**
    ...
    - [x] *Phase 5E (Tier 1) Sky, Weather & Celestial Subsystem (5 Modular Chunks):*
      ...
      - [x] **Step: Weather Emitters & Point Lights:** Looping ambient routines at their authored total length matched retail side by side on Bibiki Bay (2026-09-24).
        ...
        - [x] *Indoor sub-environment lighting & point-light power (2026-09-25):* placements linked to a sub-environment (ZoneDef +0x4C, e.g. Metalworks' 627 `ev01` / 118 `ev02` groups) are lit by that environment's own 0x2F keyframes (per-environment scene uniform sets; indoor light direction decoded from the moon slot's signed bytes) instead of the outdoor weather, which had lit Metalworks' interiors with noon daylight. Point lights saturate per channel: colour = min(half-range colour x2 x power, 1) x 0.75, so power decides how pale a light is rather than scaling it (Metalworks' always-on power 12-40 orange lights wash to a pale yellow, San d'Oria's power-2 gold lamps stay gold); calibrated against Windower captures of both zones, where linear power made Metalworks 6-20x too bright and orange (the client's exact light pipeline is not decoded). Actors follow the same rule (2026-09-25): each entity probes the floor under its feet (`ZoneRaycaster.FindFloor`, re-cast after half a yalm of movement) and, on a floor linked to a sub-environment, takes that environment's lights, so characters in Bibiki Bay's entrance cave (`ev01`: sun/ambient ~0.27/0.31, blue-tinted) no longer glow with the outdoor noon light. Actors are lit with the 0x2F model lighting block (sun, moon, ambient) at twice the 0x80-neutral strength (`ActorLighting`, scale 2: character meshes carry no vertex colours): Windower captures in Bibiki Bay showed retail characters 2.6-3x brighter at night outdoors (22:46) and 2-2.5x in the entrance cave, matching the prediction; the summed actor light is capped at 0.75 (1.5x the texel, the pre-doubling noon peak) because at noon the doubled lights flattened skin tones to twice the texel; checked in-game across the F10 presets (day, dusk, night, overcast).
    ...
    - [ ] *Tier 2 (Stock FFXI 2D UI):* Authentic DAT-driven menu boxes (blue marble), finger cursor hand, targeting brackets, vitals gauges, status icons, and dialog text.
      - [x] **Chunk 1: UI data layer.** `UiElementGroupDecoder` (Section 0x31: texture list, images of 61-byte parts = destination quad TL/TR/BL/BR, source rect, flags, per-corner RGBA colours in the 0x80 half-scale convention, sampled texture id), `UiMenuDecoder` (Section 0x30: frame with 2-bit screen anchor, buttons with 4-way nav links, 20-byte shape references to group images, help/title text ids) and `UiResourceLibrary` (English menu DAT 39542 = ROM/119/51, base menu DAT 1 = ROM/0/1 for fonts it omits, window skins 14-21 = ROM/0/14-21 overriding `newtex`/`hfr1`/`corner`/`vfr1`; textures decode lazily). Every group and menu in the retail DATs decodes to within its section padding; 3625 of 3630 shape references resolve (the rest are lobby-only); menus are authored in the 512 x 448 PS2 layout; UI textures wrap (window backgrounds tile `newtex`); menu labels are pre-rendered sprites (`windowps`), and `fontshp` glyphs run from ASCII 0x20. A software composite of the log, party, target and main-menu windows matches the retail look.
      - [x] **Chunk 2: Tier 2 sprite renderer & layout model.** `StockUiRenderer` draws element-group images and text as batched screen-space quads after the 3D pass (`RenderTier2_StockUi`): per-corner vertex colours doubled from the 0x80 half scale, texture alpha normalized on upload (DXT3 UI sheets peak at 0x88, DXT1 at 0xFF; both mean opaque), bilinear wrap sampling. Window backgrounds fade to half brightness and half opacity toward the bottom through their authored corner colours (0x7F top, 0x40 bottom), which is the retail translucency. `StockUiLayout` (Core) resolves every persistent window from its authored position and anchor corner plus an optional per-window override (anchor, position, scale, hidden), clamped on screen; keys are logical window ids (`StockUiWindowIds`) because windows swap layouts at runtime (party `ptw1`..`ptw6` by member count, log `log1`..`log8` by line count). Default scale 1 = the retail PC layout (one screen pixel per layout pixel). `StockUiLayoutStore` shares one layout per character (`ui_layouts/<name>.json`, saved on change) between the HUD and `/uilayout [scale n | reset]` / `/uilayout <window> <hide|show|reset|scale n|move x y [anchor]>`; `MoveTo` (for chunk 4b) re-anchors to the nearest corner. `UiFont` measures and draws `fontshp` glyphs. Compared against Windower captures (2026-09-25; retail draws the UI at one screen pixel per layout pixel at 2560 x 1440): part corner colours are stored bottom row first, so windows are translucent at the top; the attributes' second byte is the part blend mode (2 = darken, used by text shadows, the band behind window titles and separators; 1 = add); the client draws its own window border (the skin's `hfr1` strip along the top and bottom edges, added at ~85% so the bottom line reads lavender over the navy, fading over 16 px at the ends). The party window uses `ptw0` (titled Solo) outside a party; the log window stretches right to meet the party window (2 px gap) unless moved. Party rows (`StockUiPartyWindow`) are composed by the client, not the DAT: name, HP number (hpcol colours by HP%) and MP number in `fontshp` at 7/8 size (the glyphs carry their own dark outline; the client adds no shadow) (names that do not fit before a four-digit HP number keep their longest fitting prefix plus "..", as retail shows "Tarudrake" as "Tarudra.."), the HP gauge (5 x 5 knob + strip middle stretched to 64 px) and MP gauge (unstretched strip) from the `gauge` texture tinted by measured fills, and the party-leader ball; positions within a pixel of the capture. Opt-in (not retail): party TP under the name (`/uilayout tp on`). Party vitals: member HP/MP/TP changes arrive as S2C 0x0DF and now update `PartyState` (previously only 0x0DD at roster changes, so members froze), roster tables (0x0C8) no longer zero stored vitals, and the local player's row reads its own live vitals. Verified offscreen on D3D11 (`StockUiRendererTests`); the depleted-gauge look was accepted in-game.
      - [ ] **Chunk 3: Live HUD.**
        ...
        - [ ] Targeting cursor / brackets in the 3D view, alliance windows (needs captures), expiring-icon blink (needs 0x063 durations).
      ...
      - [ ] **Chunk 4b: Movable stock UI (opt-in).** An unlock mode (locked by default, legacy parity: retail cannot drag stock windows) that outlines the persistent windows (log, chat input, party/alliance, target, status icons, main menu) and lets the player drag them, writing the chunk 2 overrides. Runtime-placed windows (sub-menus, yes/no, dialog boxes) follow their parent rather than taking overrides; rearranging a window's contents is left to ImGui replacements.
      - [ ] **Chunk 5: Chat/log window.** Log window with channel colours and scrollback, chat input line routed to the existing slash-command dispatcher.
      - [ ] **Chunk 6: Dialog text.** Event message boxes and choice menus.
...
- [x] **Phase 5F: World Collision, Terrain Navigation & Ground Physics (MVP Completion — Blocking):**
  ...
  - [x] **Elevators & Moving Platforms:** ZoneDef placements whose BlockID (+0x34, xi-tools) starts with `@` are elevator parts, pulled out of the static scenery into `MovingPlatformGroups`. Each platform (FourCC, footprint, authored floor) travels to the landing with the most floor area beside its shaft (Metalworks' lifts 2.0 <-> -10.0, within 0.04 of a Windower capture), built by both the full and the collision-only loader. LSB's elevator `0x00E` (look size 3) carries the platform FourCC, the leg's start (Earth seconds since the Vana'diel epoch) and travel time; animation 10/11 = up/down. An elevator entity drives the lift whose footprint holds (or is nearest to) it, not the one its FourCC names (LSB's Metalworks data names each lift by the other shaft's BlockID; retail moves the lift beside the entity). `MovingPlatforms` evaluates the live height at the captured retail speed (1.99 yalms/s, a 6-second climb inside LSB's 8-second leg), a leg seen arriving plays from its arrival (updates land 1.5-2.9 s after their whole-second stamp), a shaft is closed to walkers unless its platform is at their level (retail's shaft doors), the renderer draws the parts there, the local player rides the platform (idle too), ground queries treat it as a floor, and other characters standing on it are drawn riding it (the capture showed a stationary rider carried up and down). Doors (`_` BlockIDs, open/close routines) are still drawn static.

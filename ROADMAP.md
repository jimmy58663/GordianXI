...
- [ ] **Phase 5E: UI Layering, Stock DAT 2D HUD & ImGui In-Game Overlays:**
  - [ ] **3-Tier Rendering Architecture:**
    ...
    - [x] *Phase 5E (Tier 1) Sky, Weather & Celestial Subsystem (5 Modular Chunks):*
      ...
      - [ ] **Step: 0x1F Particle-Mesh Water Surfaces:** Bibiki's sea (`umi1`) and most surf (`shi1`-`shi4`, `hna*`, `hum*`, `mizu`) are Section 0x1F ParticleMesh geometry linked from 0x05 generators; `ZoneDataLoader` only resolves 0x2E templates, so these are never drawn. Decode 0x1F and instantiate them (with generator color/alpha curves) to restore the open sea.

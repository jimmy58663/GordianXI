...
- [ ] **Phase 5E: UI Layering, Stock DAT 2D HUD & ImGui In-Game Overlays:**
  - [ ] **3-Tier Rendering Architecture:**
    ...
    - [x] *Phase 5E (Tier 1) Sky, Weather & Celestial Subsystem (5 Modular Chunks):*
      ...
      - [ ] **Step: Remaining Particle Coverage:** Unimplemented opcodes seen in zone water (0x07 spherical position variance, 0x3B incremental rotation, 0x48 double-range fade); daylight tint uses terrain sun/moon rather than the 0x2F model lights; weather-directory emitters (rain, snow) and child generators are not run. Looping ambient routines at their authored total length is inferred from retail data (the reference viewer does not play zone routines) - verify against the client.

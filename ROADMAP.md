  ...
  - [ ] **3-Tier Rendering Architecture:**
    ...
    - [ ] *Phase 5E (Tier 1) Sky, Weather & Celestial Subsystem (5 Modular Chunks):*
      ...
      - [x] **Chunk 3: Celestial Night Sky (Stars & Moon):**
        ...
        - [ ] Follow-ups: `pole` star (sprite sheet `hit6`) and moon lens flare (`molf`, lens-flare sprite sheet) are not yet drawn.
      - [ ] **Chunk 4: Dynamic Cloud Shells [Next Focus]:** Weather-gated dynamic cloud layers (`clod`, `suny`, `fine`, `mist`) drifting with authentic UV velocities and daylight-modulated ambient lighting. Cloud shells now render correctly after the Chunk 3 shader-interface fix (`EnableWeatherClouds = true`); remaining work is moving them onto the `DrawCelestialGenerator`-style generator path (multiple generators per mesh, `0x19` color/alpha curves, authored blend) and verifying drift speeds.

  ...
  - [ ] **3-Tier Rendering Architecture:**
    ...
    - [ ] *Phase 5E (Tier 1) Sky, Weather & Celestial Subsystem (5 Modular Chunks):*
      ...
      - [x] **Chunk 4: Dynamic Cloud Shells:**
        ...
        - [x] All particle blend functions (`ParticleBlendFunc`): alpha, additive, reverse-subtract (new pipeline; darkens for the `ykum` sunset band) and darken-by-alpha; painter's order by generator projection bias (`0x30`: stars 50000 → clouds 40000–28000 → moon 20000 → flare).
      ...
      - [ ] **Chunk 5: Solar & Horizon Alignment:** Daytime solar disc (`sunsphere`), radiant corona flare, and horizon alignment opposite the moon along the seasonal ecliptic arc.
        - Finding from Chunk 4: `VanaTime.GetSunDirection` is mirrored relative to the client. xi-model-viewer's `getSunPosition` (after xim) puts sunrise at display -X and sunset at display +X; ours rises at +X. The authored `ykum` sunset cloud band confirms the client orientation (it sits on the +X horizon), so the sun (terrain lighting direction) and moon (`-sunDir`) currently rise and set on the wrong sides.
        - Sun lens flares (`lf01`-`lf03`, 16-26 sprites strung across the screen) need an all-or-nothing visibility test; the moon flare's per-pixel terrain occlusion is not sufficient for them.
